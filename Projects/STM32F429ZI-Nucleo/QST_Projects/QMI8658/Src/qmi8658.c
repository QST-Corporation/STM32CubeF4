...
void MX_SPI1_Init(void)
{
  ...
  hspi1.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_8;//SPI_BAUDRATEPRESCALER_16;
...
static HAL_StatusTypeDef QMI8658_SPI_Read(uint8_t reg, uint8_t *pData, uint16_t size)
{
  ...
  Status = HAL_SPI_TransmitReceive_DMA(&hspi1, txData, rxData, size+1);
  delay_us(50);
...
static HAL_StatusTypeDef QMI8658_SPI_Write(uint8_t reg, uint8_t *pData, uint16_t size)
{
  ...
  memmove(&payload[1], pData, size);
  Status = HAL_SPI_Transmit_DMA(&hspi1, payload, size+1);
...
unsigned char stepConfig(unsigned short odr)  //11hz=0x0e锟斤拷
{
...
void QMI8658_Sensor_Test(void)
{
  ...
  float gyro[3] = {0,};
  float sample[6] = {0};
  static float sampleMoveUnit[20][6] = {0};
